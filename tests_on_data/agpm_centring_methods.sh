...
rm rc_*.fits
mpirun $GRAPHIC_N_CORES python $GRAPHIC_DIR"GRAPHIC_naco_register_"$GRAPHIC_VERSION".py" --pattern o_ -naco -remove_striping --search_region 50 >> $OUTPUT_FILE
mpirun $GRAPHIC_N_CORES python $GRAPHIC_DIR"GRAPHIC_recenter_cubes_"$GRAPHIC_VERSION".py" --pattern nomed_ --info_pattern all_info_ --naxis3 248 --lmax 600  >> $OUTPUT_FILE
...
rm cube-info/all_info_*.rdb # delete the o_ info files and create new ones
rm rc_*.fits
mpirun $GRAPHIC_N_CORES python $GRAPHIC_DIR"GRAPHIC_naco_register_"$GRAPHIC_VERSION".py" --pattern nomed_ -naco -agpm_centre --search_region 50 >> $OUTPUT_FILE
mpirun $GRAPHIC_N_CORES python $GRAPHIC_DIR"GRAPHIC_recenter_cubes_"$GRAPHIC_VERSION".py" --pattern nomed_ --info_pattern all_info_ --naxis3 248 --lmax 600  >> $OUTPUT_FILE
...
cd Targ
rm rc_*.fits
mpirun $GRAPHIC_N_CORES python $GRAPHIC_DIR"GRAPHIC_recenter_cubes_"$GRAPHIC_VERSION".py" --pattern nomed_ --info_pattern all_info_ --naxis3 248 --lmax 600 -nofit  >> $OUTPUT_FILE
